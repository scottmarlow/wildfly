            ...
            while (!notRolledBackException) {
                try {
                    ...
                    if (exception instanceof EJBTransactionRolledbackException) {
                        System.out.println("xxx ignoring expected RollbackException by repeating invocation, count=" + entityCount);
                    } else {
                        System.out.println("xxx caught exception that we didn't expect: " + exception.getClass().getName() + ", " + exception.getMessage() +
                                ", count=" + entityCount);
