...
/**
 * MongoDriverScanDependencyProcessor
 *
 * @author Scott Marlow
 */
public class MongoDriverScanDependencyProcessor implements DeploymentUnitProcessor {
    ...
    // there should be no more than one MongoDB module referenced (there can be many references to that module but only
    // one version of MongoDB should be included per application deployment).
    ...
    protected void processFieldResource(final DeploymentUnit deploymentUnit, final String lookup) throws DeploymentUnitProcessingException {
        ...
        String moduleName = MongoSubsystemService.getMongoSubsystem().moduleName(lookup);
    ...
    private static void savePerDeploymentModuleName(DeploymentUnit deploymentUnit, String module) {
        ...
        synchronized (deploymentUnit) {
            ...
            if (currentValue == null) {
                ...
                // TODO: Add logger
                throw new IllegalStateException("Cannot add reference to MongoDB module='" + module + "' when module '" + currentValue +
