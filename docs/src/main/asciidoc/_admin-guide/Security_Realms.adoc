...
access as exposed by JBoss Remoting such as remote JNDI and Jakarta Enterprise Beans access,
...
comunications with JNDI and the Jakarta Enterprise Beans subsystem by default we associate
...
these are typically used for remote Jakarta Enterprise Beans invocations from one AS server
...
over the same Remoting connector used for remote JNDI and Jakarta Enterprise Beans access -
