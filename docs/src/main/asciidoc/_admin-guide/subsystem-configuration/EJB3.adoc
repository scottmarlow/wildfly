...
This element enables async Jakarta Enterprise Beans invocations. It is also used to specify
the thread pool that these invocations will use.

[[timer-service]]
== <timer-service>

This element enables the Jakarta Enterprise Beans timer service. It is also used to specify
the thread pool that these invocations will use.
...
This is used to enable remote Jakarta Enterprise Beans invocations. It specifies the remoting
...
by the Jakarta Enterprise Beans spec. This element can use used to enable pass by reference,
which can give you a performance boost. Note WildFly will do a shallow
check to see if the caller and the Jakarta Enterprise Beans have access to the same class
...
class definition on the call & Jakarta Enterprise Beans side. If the top level class
