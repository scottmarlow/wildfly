[[EJB_Services]]
= Jakarta Enterprise Beans Services
...
link:Developer_Guide{outfilesuffix}#EJB_invocations_from_a_remote_client_using_JNDI[JNDI approach] or native JBoss Jakarta Enterprise Beans client APIs to
communicate with the servers. The important thing to note is that when
you are invoking clustered Jakarta Enterprise Beans deployments, you do *not* have to list
...
When a client connects to a server, the JBoss Jakarta Enterprise Beans client implementation
...
_affinity_ will later help the Jakarta Enterprise Beans client to route the invocations on
...
JBoss Jakarta Enterprise Beans client implementation will take note of this topology
...
proxy. The JBoss Jakarta Enterprise Beans client implementation, at this stage will be aware
...
node from among the cluster, the JBoss Jakarta Enterprise Beans client implementation creates
...
EJB client implementation will use it to create a Jakarta Enterprise Beans receiver out of it
...
the Jakarta Enterprise Beans client API or the jboss-ejb-client.properties based approach to
