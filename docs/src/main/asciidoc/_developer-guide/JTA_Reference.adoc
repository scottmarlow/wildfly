...
* first - Jakarta Enterprise Beans annotations, their meaning and capabilities are defined under
  Jakarta Enterprise Beans specification and are valid when used in the Jakarta Enterprise Beans component
...
or with Jakarta Enterprise Beans `@Resource` injection where you can additionally define the JNDI
...
Transaction management in Jakarta Enterprise Beans is administered with two annotations:
...
The default behaviour of an Jakarta Enterprise Beans is to be

* container managed - transaction boundaries are driven by annotations
* when the Jakarta Enterprise Beans method is invoked - a new transaction is started when no transaction context is available,
or the method joins the existing transactions when the transaction context is passed by the call

It's the same transactional behaviour as if the Jakarta Enterprise Beans is annotated with
...
For registration of the synchronization callback, the use can inject the
