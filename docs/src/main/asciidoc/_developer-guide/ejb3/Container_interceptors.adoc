...
that those interceptors get invoked during an Jakarta Enterprise Beans invocation. Such
...
duties. As a result, these Jakarta EE interceptors come too late into the
...
== Typical Jakarta Enterprise Beans invocation call path on the server

A typical Jakarta Enterprise Beans invocation looks like this:
...
6.  Invocation on the Jakarta Enterprise Beans instance's method
...
container interceptors. So we decided to allow the Jakarta EE interceptors
...
configuring the Jakarta EE interceptors for EJBs. In fact, it uses the same
...
EJB3.1 xsd allows for standard Jakarta EE interceptors
...
The container interceptors configured for a Jakarta Enterprise Beans are guaranteed to be run
...
Jakarta EE interceptors, there are some differences in the API semantics.
...
the Jakarta Enterprise Beans components are setup or instantiated.
