...
= Jakarta Enterprise Beans invocations from a remote client using JNDI
...
EJB invocations via JNDI - Jakarta Enterprise Beans client API or remote-naming project]

Before getting into the details, we would like the users to know that we
have introduced a new Jakarta Enterprise Beans client API, which is a WildFly-specific API
...
separate document covering the Jakarta Enterprise Beans remote client API will be made
available. For now, you can refer to the javadocs of the Jakarta Enterprise Beans client
...
WildFly specific Jakarta Enterprise Beans client API to do the invocation or use JNDI to
...
leave the Jakarta Enterprise Beans client API for a separate chapter.
...
 
/**
 * A sample program which acts a remote client for a Jakarta Enterprise Beans deployed on Wildfly 10 server.
...
 
    ...
    private static RemoteCalculator lookupRemoteStatelessCalculator() throws NamingException {
        ...
        // Jakarta Enterprise Beans deployment on the server.
        // Since we haven't deployed the application as a .ear, the app name for us will be an empty string
        final String appName = "";
        // This is the module name of the deployed EJBs on the server. This is typically the jar name of the
        // Jakarta Enterprise Beans deployment, without the .jar suffix, but can be overridden via the ejb-jar.xml
        // In this example, we have deployed the EJBs in a jboss-as-ejb-remote-app.jar, so the module name is
        // jboss-as-ejb-remote-app
        final String moduleName = "jboss-as-ejb-remote-app";
        // AS7 allows each deployment to have an (optional) distinct name. We haven't specified a distinct name for
        // our Jakarta Enterprise Beans deployment, so this is an empty string
        final String distinctName = "";
        // The Jakarta Enterprise Beans name which by default is the simple class name of the bean implementation class
...
first step in the client code, we'll do a lookup of the Jakarta Enterprise Beans using a JNDI
...
The ejb: namespace identifies it as a Jakarta Enterprise Beans lookup and is a constant (i.e.
doesn't change) for doing Jakarta Enterprise Beans lookups. The rest of the parts in the jndi
name are as follows:
...
and the Jakarta Enterprise Beans client API implementation doesn't contact the server during
...
it uses. Since our stateless Jakarta Enterprise Beans named CalculatorBean is deployed in a
...
For the stateful Jakarta Enterprise Beans named CounterBean which is deployed in hte same
...
implementation (which is backed by the Jakarta Enterprise Beans client API) needs to know the
...
== Setting up Jakarta Enterprise Beans client context properties

A Jakarta Enterprise Beans client context is a context which contains contextual information
for carrying out remote invocations on EJBs. This is a WildFly-specific
API. The Jakarta Enterprise Beans client context can be associated with multiple EJB
receivers. Each Jakarta Enterprise Beans receiver is capable of handling invocations on
different EJBs. For example, an Jakarta Enterprise Beans receiver "Foo" might be able to
handle invocation on a bean identified by
app-A/module-A/distinctinctName-A/Bar!RemoteBar, whereas a Jakarta Enterprise Beans receiver
named "Blah" might be able to handle invocation on a bean identified by
app-B/module-B/distinctName-B/BeanB!RemoteBean. Each such Jakarta Enterprise Beans receiver
knows about what set of EJBs it can handle and each of the Jakarta Enterprise Beans receiver
...
JBoss specific Jakarta Enterprise Beans remote client protocol (details of which will be
explained in-depth in a separate chapter).

Now that we know what a Jakarta Enterprise Beans client context is and what a Jakarta Enterprise Beans receiver
is, let's see how we can setup a client context with 1 Jakarta Enterprise Beans receiver
...
receivers will communicate with the server for Jakarta Enterprise Beans invocations.
...
each of the connections will map to a Jakarta Enterprise Beans receiver. So if you have 2
connections, that will setup 2 Jakarta Enterprise Beans receivers that will be added to the
...
By default WildFly uses 8080 as the remoting port. The Jakarta Enterprise Beans client API
...
connections will internally be used to create the Jakarta Enterprise Beans receivers in the
EJB client context.
...
The Jakarta Enterprise Beans client code will by default look for jboss-ejb-client.properties
...
In the above examples, we saw what it takes to invoke a Jakarta Enterprise Beans from a
