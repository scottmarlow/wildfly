...
* Jakarta EE 8 - {ProductName} is a certified implementation of Jakarta EE 8, meeting both the Web and the Full profiles, and already includes support for the latest iterations of CDI (1.2) and Web Sockets (1.1).
...
* RESTEasy 3 - {ProductName} includes the latest generation of RESTEasy, which goes beyond the standard Jakarta EE REST APIs (JAX-RS 2.0), by also providing a number of useful extensions, such as JSON Web Encryption, Jackson, Yaml, JSON-P, and Jettison.
...
WebSockets are a requirement of the Jakarta EE 8 specification and the default configuration is included in {ProductName}. More complex WebSocket configuration is done in the `servlet-container` of the `undertow` subsystem of the server configuration file.
...
Container-Managed Persistence entity beans support is optional in Jakarta EE 8, and {ProductName} does not provide support for these.
...
The proprietary XML descriptors, previously used to setup JMS resources, are deprecated in {ProductName}. Jakarta EE 8 (section EE.5.18) standardized such functionality.
...
Any application containing a `jbossws-cxf.xml` must migrate all functionality specified in such XML descriptor, mostly already supported by the JAX-WS specification, included in Jakarta EE 8. It is still possible to rely on direct Apache CXF API usage, loosing the Jakarta EE portability of the application, for instance when specific Apache CXF functionalities are needed. See the Apache CXF Integration document for further information.

[[jax-rpc]]
==== JAX-RPC

JAX-RPC is an API for building Web services and clients that used remote procedure calls (RPC) and XML, which was deprecated in Jakarta EE 8, and is no longer supported by {ProductName}.
