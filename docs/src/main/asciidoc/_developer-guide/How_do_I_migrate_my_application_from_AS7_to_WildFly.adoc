...
* Jakarta EE - {ProductName} is a certified implementation of Jakarta EE, meeting both the Web and the Full profiles, and already includes support for the latest iterations of CDI (1.2) and Web Sockets (1.1).
...
* RESTEasy 3 - {ProductName} includes the latest generation of RESTEasy, which goes beyond the standard Jakarta RESTful Web Services APIs, by also providing a number of useful extensions, such as JSON Web Encryption, Jackson, Yaml, JSON-P, and Jettison.
...
* CMP Jakarta Enterprise Beans - JPA offers a much more performant and flexible API
...
JAX-RPC is an API for building Web services and clients that used remote procedure calls (RPC) and XML, which was deprecated in Jakarta EE, and is no longer supported by {ProductName}.
...
Applications using JAX-RPC should be migrated to use JAX-WS, the current Jakarta EE standard web service framework.

[[REST]]
==== Jakarta RESTful Web Services

JSR 339: The Java API for RESTful Web Services specification is located at https://jcp.org/en/jsr/detail?id=339

Some changes to the `MessageBodyWriter` interface may represent a backward incompatible change with respect to 1.X.
...
{ProductName} {ProductVersion}'s default JNDI Provider URL has changed, which means that external applications, using JNDI to lookup remote resources, for instance an Jakarta Enterprise Beans or a JMS Queue, may need to change the value for the JNDI `InitialContext` environment's property named
