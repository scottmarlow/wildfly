...
invoke an Jakarta Enterprise Beans deployed on a remote server using a
...
An Jakarta Enterprise Beans can then be looked up and a method can be invoked on it as
follows:
...
InitialContext context = new InitialContext(properties);
 
// look up an Jakarta Enterprise Beans and invoke one of its methods (same as before)
RemoteCalculator statelessRemoteCalculator = (RemoteCalculator) context.lookup(
    "ejb:/ejb-remote-server-side//CalculatorBean!" + RemoteCalculator.class.getName());
int sum = statelessRemoteCalculator.add(101, 202);
----
